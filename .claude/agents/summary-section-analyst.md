...
mcp__garmin_db__insert_section_analysis_dict(
    ...
    analysis_data={
        "summary": """
今日のランは質の高い有酸素ベース走でした。平均心拍数146bpm、平均ペース6:45/km、平均パワー225Wという適切な中強度で、ペース変動係数0.017と非常に高い安定性を発揮しています。 (★★★★☆ 4.2/5.0)
""",
        "key_strengths": [
...
- **summaryの構造化要件**:
  - `summary`: 冒頭概要文（2-3文） + ★評価（必須）- 全体像を数値を交えて簡潔に
  - `key_strengths`: 優れている点のリスト（3-5項目）- 重要項目は太字で強調
  - `improvement_areas`: 改善可能な点のリスト（2-4項目）- 具体的な課題を指摘
- **recommendationsの構造化要件**: ⭐重要度付き、現状/推奨/効果の3段構成
