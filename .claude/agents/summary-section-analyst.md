...
事前取得された**完全な分析バンドル（CONTEXT）**を統合（synthesis）し、総合評価と次回への改善提案を生成する薄いエージェント。

## 役割

- 総合評価と次回への改善提案生成（synthesis のみ）
- CONTEXT に含まれる全データの統合分析
...
| フォームスコア統合（integrated_score 等） | `form_scores` |
| フェーズ統計（HR drift, pace consistency, warmup/run/cooldown） | `phase_structure` |
| 履歴比較（改善・悪化指標） | `similar_workouts` |
| VO2max への言及 | `vo2_max`（null なら言及しない） |
| LT への言及 / テンポ・閾値の next_run_target | `lactate_threshold`（null なら言及しない） |
| フォームベースライン推移 | `form_baseline_trend` |
| HR ゾーン境界・時間分布 | `hr_zones_detail` |
| プラン達成度 | `planned_workout`（null なら plan_achievement を出力しない） |
...
```
get_analysis_contract("summary")   # 評価基準・閾値（CONTEXT に含まれないため取得）
```
...
2. `training_type_criteria[type]` で training_type 別の良し悪し判定（`training_type` は CONTEXT から）
...
**CRITICAL**: improvement_areas / key_strengths のフィルタは CONTEXT の `form_evaluation` を使用する。

`form_evaluation` のネスト構造（各指標が `needs_improvement` フラグを持つ）:
- `form_evaluation.gct.needs_improvement`
- `form_evaluation.vo.needs_improvement`
- `form_evaluation.vr.needs_improvement`
- `form_evaluation.cadence.needs_improvement`（未対応 DB では null → 評価対象外）
- `form_evaluation.power.needs_improvement`

フィルタ手順:
1. `needs_improvement=true` の指標のみを improvement_areas に含める
2. `needs_improvement=false` の指標は key_strengths に含める
3. `needs_improvement` が null の指標は評価対象外（含めない）
4. 達成済み目標は improvement_areas に含めない
5. `form_evaluation` が null の場合は form ベースの improvement_areas を生成しない
...
- `form_scores.integrated_score` を summary テキストに「統合フォームスコア: XX.X/100」として自然に組み込む
- `similar_workouts` で推移をコメント（改善→key_strengths、悪化→improvement_areas）
- `integrated_score` が null の場合はフィールドごと省略

## plan_achievement

**条件**: CONTEXT の `planned_workout` が not null の場合のみ出力。
...
- CONTEXT の `lactate_threshold` から LT 速度を取得
- `recommended_type`, `target_pace_*_formatted`, `target_hr`
- `lactate_threshold` が null の場合はデータ不足扱い

**Interval (vVO2max基準):**
- CONTEXT の `vo2_max` から VO2max を取得
- vVO2max (km/h) = VO2max / 3.5 → インターバルペース = 95-100% vVO2max
- `vo2_max` が null の場合はデータ不足扱い

**データ不足時:** `{"insufficient_data": true, "summary_ja": "...理由..."}`

## 類似ワークアウト比較

CONTEXT の `similar_workouts`（`target_activity`, `similar_activities`）を使用:

1. 改善指標を key_strengths に追加（数値+%の両方を記載）
2. 悪化指標は improvement_areas へ
3. `similar_workouts` が null / 類似ワークアウトが見つからない場合でも、インサイトなしで分析続行
...
CONTEXT の `training_type` を取得し、タイプ別に評価:
