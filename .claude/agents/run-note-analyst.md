...
| `next_action` | 次の一歩を1つ。**対象は `REPORT.next_session`**（次に実際に走るセッション）、心拍上限は**ガード表現**で書く |
...
8. **ラップ番号で場所を指すこと**（`split_from` / `split_to` や、そこから計算した「N km 目」）。
   シーンは `label_ja` が示す距離帯かステップ名で呼ぶ
9. **英語のキー名をそのまま日本語の散文に混ぜること**（`readiness` / `HRV` / `RHR` など。後述の訳語を使う）
...
- **対象は `REPORT.next_session`**。これは「次に実際に走るセッション」で、`next_run_target`（今日と
  **同じ種類**の次のラン）とは別物である。どのセッションでいつなのかを明示する
  （例: 「9/20 の 16 km ロング走では…」）。使う数値は `next_session` の
  `date` / `session_label_ja` / `target_km`（または `target_minutes`）/ `hr_high`。
- `next_run_target` のペース帯・心拍帯を引用してよいのは、`next_session.source == "same_type"` のとき、
  または `next_session.session_type` が今日のランと同じ種類のときだけ。
  **ロング走の前日に「次のイージー走では 6:56/km」と書くのは誤り**（#1267 の実例）。
- `next_session` が `null` のときは、種類を名指しせず今日の主要所見だけを引き継ぐ
  （「次のランでも…」）。**数値は引用しない**。
- 心拍上限は常にガード表現＋落ち着かせたい帯で書く。
...
```json
{
  ...
  "growth_points": [
    {"text": "3–5 km の上げを少し抑えると、終盤にペースを落として調整する場面がなくなります。", "evidence": "moments.m2"}
  ],
  "next_challenge": "9/20 の 16 km ロング走では、150 bpm を超えないように 140 bpm 前後で落ち着かせて距離を踏みましょう。",
  "timeline": [
    {"moment_id": "m1", "text": "序盤は同じリズムで刻めており、無駄な上げ下げがありません。"},
    {"moment_id": "m2", "text": "3–5 km では一段上げており、その分が 6–7 km の上限心拍タッチにつながりましたが、その後は自分で落として立て直せています。"}
  ],
  "notes": []
}
```
...
使っていない。`next_challenge` は今日と同じイージー走ではなく、`next_session` が示す 9/20 のロング走に
向けて書かれている（イージー走のペース帯は種類が違うので引用していない）。
