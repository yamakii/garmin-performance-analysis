...
**低～中強度走の評価基準:**
```
- ペース安定性: 変動係数<0.05が目標、<0.08が許容範囲（リラックスした走りのため緩め）
...
**閾値走の評価基準:**
```
- ペース安定性: 変動係数<0.03が目標（excellent）、<0.05が許容範囲
- Zone 4時間比率: 60%以上が理想
...
**インターバルの評価基準:**
```
- Work区間ペース安定性: 変動係数<0.04が目標（Work セグメント内のみで評価、全体CVは評価しない）
...
(★★★★☆ 4.0/5.0)
""",
        "evaluation_criteria": "- Work区間ペース安定性: 変動係数<0.04が目標（Workセグメント内のみで評価、全体CVは評価しない）\n- Recovery心拍回復: Work心拍の70-80%まで下がることが理想\n- インターバル本数: 5-10本が一般的（距離・強度による）\n- Zone 4-5時間比率: 40%以上が目標（高強度トレーニング）"
    }
...
(★★★★☆ 4.0/5.0)
""",
        "evaluation_criteria": "- ペース安定性: 変動係数<0.03が目標（excellent）、<0.05が許容範囲\n- Zone 4時間比率: 60%以上が理想\n- 心拍ドリフト: 15-25%が正常範囲（追い込むメニューのため）\n- メイン区間持続: 20-30分が理想的"
    }
    }, ensure_ascii=False, indent=2)
)
```
...
### Run/Main
- ペース安定性（training type 別）:
  - low_moderate: CV<0.05 = excellent（easy/recovery は自然なペース変動を許容）
  - tempo_threshold: CV<0.03 = excellent
  - interval_sprint: Work/Recovery 各セグメント内で CV<0.04（全体 CV は評価しない）
- HR drift <10% = 理想的な疲労管理（interval は 15-25% が正常）
