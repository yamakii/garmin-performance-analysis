...
4. 各異常をスプリット番号と紐づけ、該当スプリットの解説に原因を統合
   - 変化は**絶対量で淡々と**記述する（例「GCTが12ms増」「VOが0.4cm増」）。「急激」「崩壊」「崩れ」等の誇張語は使わない
   - **fatigue**: 「GCTが12ms増 - 後半にかけての持続的悪化を伴う。ヒップドライブを意識」（fatigue ラベルは持続的悪化トレンドがある異常にのみ付く）
   - **elevation_change**: 「VOが0.5cm増 - 上り区間の影響。地形対応として正常範囲」
   - **pace_change**: 「VRが0.4%増 - ペース変化に伴う一時的なゆらぎ。単発であれば実用上問題なし」
...
**フォーム異常の記述方針（重要・Issue #232）:**
- 単発・小幅の異常を「急激」「崩壊」「崩れ」等の誇張語で表現しない。変化は**絶対量で淡々と**記述（例「VOが0.4cm増」「GCTが12ms増」）し、その実用上の重要度（material かどうか）を先に判断する
- 「持続的なフォーム維持に課題」等の**全体結論は、メイン区間の前半 vs 後半で `form_degradation_triggers`（VO+0.5cm / GCT+10ms / VR+0.3%）を超える持続的悪化が実在する場合のみ**述べる。散発的な点異常から「持続的な課題」「疲労性のフォーム崩れ」と結論づけない
- 重要でない小さなゆらぎは個別に列挙せず、「後半に軽微なゆらぎはあるが実用上問題なし」程度に集約する
