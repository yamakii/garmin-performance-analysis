...
| `get_weekly_review` | 引数なし | **今週の処方**（週量・ロング・カットバック判定・回復ゲート）。これが判定の背骨 |

`get_load_trend(lookback_weeks=6)` は「今週何 km」「ロングを伸ばしていいか」など**週単位の量**を聞かれたときだけ追加します。
`get_athlete_profile` は `get_weekly_review` が null のとき、または目標・フェーズ自体が論点のときだけ読みます（focus_notes は長大なので毎日は読まない）。

## Step 3: 判定の組み立て

1. **背骨は週次レビューの処方**。今日の判断は必ず `get_weekly_review` の recommendations / 週量 / ロング計画 / カットバック判定と整合させる。処方と矛盾する提案をする場合は、その旨と理由を明示する（黙って別案を出さない）
