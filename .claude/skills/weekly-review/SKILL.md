...
- **バンドルの決定的な値は再計算しない**: `prescriptions_prev_week.adherence`（`catch_up_ingest` の突き合わせ結果）や `load_trend.long_run` の値はそのまま引用する。`cutback_due=true` なら、進行ゲートが GREEN でも回復指標が全て緑でも deload を優先する。
