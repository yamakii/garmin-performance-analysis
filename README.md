...
- **Garmin MCP Integration**: 52 token-optimized MCP tools for data retrieval and analysis, declared from a single-source `tools/` registry ([tool reference](docs/mcp-tools-reference.md))
...
- **`packages/garmin-mcp-server`**: Python MCP server
  ...
  - `src/garmin_mcp/tools/`: `ToolDef` registry — single source for all 52 MCP tools
...
- **garmin-db**: DuckDB-based performance data queries and section analysis storage (52 tools)
