...
| `tools/` registry | 56 tools declared as `ToolDef` (54 domain + 2 server). `server.py` dispatches directly from `ALL_DEFS_BY_NAME` (O(1) lookup) |
...
│       │   ├── tools/              # ToolDef registry (single source for 56 MCP tools)
