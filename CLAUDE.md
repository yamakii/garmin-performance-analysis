...
- 51 token-optimized MCP tools (70-98.8% reduction), declared via a single-source `tools/` registry
...
| `tools/` registry | 51 tools declared as `ToolDef` (49 domain + 2 server). `server.py` dispatches directly from `ALL_DEFS_BY_NAME` (O(1) lookup) |
...
│       │   ├── tools/              # ToolDef registry (single source for 51 MCP tools)
