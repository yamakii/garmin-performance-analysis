...
- 52 token-optimized MCP tools (70-98.8% reduction), declared via a single-source `tools/` registry
...
| `GarminDBWriter` | DuckDB write operations (21 tables, 13 inserters) |
| `GarminDBReader` | DuckDB read operations (query builders) |
| `tools/` registry | 52 tools declared as `ToolDef` (50 domain + 2 server). `server.py` dispatches directly from `ALL_DEFS_BY_NAME` (O(1) lookup) |

**DuckDB Schema (21 domain tables):**
...
│       │   ├── tools/              # ToolDef registry (single source for 52 MCP tools)
