...
# --- Planned workout fixtures (Issue #721) -----------------------------
# Mirror the planned_workouts schema (db_writer.py + add_plan_versioning) with
# only the columns the Today card query reads. The Web app is read-only; data
# is written here directly. A date can carry two plan versions; the latest
# (highest ``version``) must win.

_CREATE_PLANNED_WORKOUTS = """
...
# (workout_id, plan_id, version, date, type, desc, dist, pace_lo, pace_hi,
#  hr_lo, hr_hi, actual_id, adherence)
_PLANNED_ROWS = [
    (
        "w-0702-v1",
        "plan-1",
...
@pytest.fixture
def planned_workouts_db_path(tmp_path: Path) -> Path:
    """DuckDB with planned_workouts: 2026-07-02 (2 versions) + 2026-07-03."""
    db_path = tmp_path / "test_garmin_web_planned.duckdb"
    conn = duckdb.connect(str(db_path))
    try:
        conn.execute(_CREATE_PLANNED_WORKOUTS)
        conn.executemany(
            "INSERT INTO planned_workouts ("
            "workout_id, plan_id, version, workout_date, workout_type,"
            " description_ja, target_distance_km, target_pace_low,"
            " target_pace_high, target_hr_low, target_hr_high,"
...
@pytest.fixture
def empty_planned_workouts_db_path(tmp_path: Path) -> Path:
    """DuckDB with an empty planned_workouts table."""
    db_path = tmp_path / "test_garmin_web_planned_empty.duckdb"
    conn = duckdb.connect(str(db_path))
    try:
        conn.execute(_CREATE_PLANNED_WORKOUTS)
    finally:
