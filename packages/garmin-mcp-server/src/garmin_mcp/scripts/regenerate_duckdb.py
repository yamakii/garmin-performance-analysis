...
def main():
    ...
    # Detect activities missing form_evaluations (not regenerated by this script)
    # and nudge the user to run reevaluate_all_activities. Skipped in dry-run.
    if args.activity_ids:
        target_ids: list[int] | None = args.activity_ids
    elif args.start_date and args.end_date:
        target_ids = [
            aid
            for aid, _ in regenerator.get_activities_by_date_range(
    ...
    missing = find_missing_form_evaluations(target_ids, regenerator.db_path)
    _print_form_evaluation_nudge(missing)


def _print_form_evaluation_nudge(missing: list[tuple[int, str]]) -> None:
    """Print a nudge to regenerate missing form_evaluations, if any."""
    if not missing:
        return

    dates = ", ".join(date for _, date in missing)
    ids = " ".join(str(activity_id) for activity_id, _ in missing)
    print(
        f"\n⚠️  form_evaluations が {len(missing)} 件未生成です"
        "（regenerate では生成されません）:"
    )
    print(f"    {dates}")
    print("    → 次を実行してください:")
    print(
        "    uv run python -m garmin_mcp.scripts.reevaluate_all_activities "
        f"--activity-ids {ids}"
    )
