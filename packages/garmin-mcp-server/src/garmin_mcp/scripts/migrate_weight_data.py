...
Usage:
    ...
    python -m garmin_mcp.scripts.migrate_weight_data.py --dry-run --all

    # Migrate all files
    python -m garmin_mcp.scripts.migrate_weight_data.py --all

    # Migrate single date
    python -m garmin_mcp.scripts.migrate_weight_data.py --date 2025-05-15

    # Verify migration
    python -m garmin_mcp.scripts.migrate_weight_data.py --verify

    # Cleanup old structure (requires successful verification)
    python -m garmin_mcp.scripts.migrate_weight_data.py --cleanup
"""

import argparse
import sys

from garmin_mcp.batch.weight_data_migrator import WeightDataMigrator
