"""Handler for athlete profile tools.

Thin adapter over the single-source registry
(``garmin_mcp.tools.athlete.ATHLETE_TOOLS``). ``default=str`` is applied when
serializing so date-bearing profile/review payloads remain JSON-encodable,
...
from garmin_mcp.tools.athlete import ATHLETE_TOOLS_BY_NAME
from garmin_mcp.tools.registry import dispatch


class AthleteHandler:
    """Handles athlete-profile-related tool calls via the registry."""

    _tool_names: set[str] = set(ATHLETE_TOOLS_BY_NAME)
