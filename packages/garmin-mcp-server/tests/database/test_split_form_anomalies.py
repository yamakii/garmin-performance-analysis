...
over synthetic raw ``activity_details.json`` and maps each anomaly's 1 Hz
...
path end to end (raw JSON -> detector -> split map) plus the two "nothing to
highlight" degradations (no raw file, no split rows).
"""
...
def _write_activity_details(base_path: Path, activity_id: int) -> None:
    ...
    for index in range(_SERIES_LEN):
        gct = 290.0 if index in _SPIKE_SECONDS else 240.0
        rows.append({"metrics": [140, 28, gct, 80, _elevation(index)]})
    payload = {
