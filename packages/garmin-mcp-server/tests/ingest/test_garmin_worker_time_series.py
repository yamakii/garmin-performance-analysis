...
class TestGarminWorkerTimeSeriesIntegration:
    ...
    def test_save_data_with_time_series(
        ...
        with (
            patch(
                "garmin_mcp.database.inserters.activities.insert_activities"
            ) as mock_activities,
            patch("garmin_mcp.database.inserters.splits.insert_splits") as mock_splits,
            patch(
                "garmin_mcp.database.inserters.form_efficiency.insert_form_efficiency"
            ) as mock_form,
            patch(
                "garmin_mcp.database.inserters.heart_rate_zones.insert_heart_rate_zones"
            ) as mock_hr_zones,
            patch(
                "garmin_mcp.database.inserters.hr_efficiency.insert_hr_efficiency"
            ) as mock_hr_eff,
            patch(
                "garmin_mcp.database.inserters.performance_trends.insert_performance_trends"
            ) as mock_perf,
            patch(
                "garmin_mcp.database.inserters.lactate_threshold.insert_lactate_threshold"
            ) as mock_lt,
            patch("garmin_mcp.database.inserters.vo2_max.insert_vo2_max") as mock_vo2,
            patch(
