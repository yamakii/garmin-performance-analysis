...
@pytest.mark.unit
def test_doc_magic_numbers_tools_75() -> None:
    """After #1218 + #1219 + #1220 the live MCP surface is 75 tools; docs/registry must agree."""
    assert _expected_tool_count() == 75
    for doc in _DOC_PATHS:
        text = doc.read_text(encoding="utf-8")
        for count in _numbers_before(r"(?:MCP )?tools", text):
            assert count == 75, f"{doc.name}: doc says {count} tools, expected 75"
