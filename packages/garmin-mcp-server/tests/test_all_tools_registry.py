...
def test_all_tools_count_49() -> None:
    """ALL_DEFS holds the 49 handler-domain tools (unique names); the live MCP
    surface adds the 2 server tools for 51 total.
...
def test_tool_count_unchanged() -> None:
    ...
    is retained by name; the surface serves 51 tools (49 domain + 2 server,
    after #499's get_recovery_trend)."""
    live_names = [t.name for t in get_tool_definitions()]
    assert len(live_names) == 51
    assert "reload_server" in live_names
    assert "get_server_info" in live_names
