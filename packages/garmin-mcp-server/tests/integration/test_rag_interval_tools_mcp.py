...
class TestRagIntervalToolsMcp:
    ...
    async def test_call_interval_analysis_with_minimal_args(
        ...
        with patch(
            "garmin_mcp.rag.queries.interval_analysis.IntervalAnalyzer.__init__",
            mock_init,
        ):
    ...
    async def test_call_interval_analysis_returns_valid_response(
        ...
        with patch(
            "garmin_mcp.rag.queries.interval_analysis.IntervalAnalyzer.get_interval_analysis"
        ) as mock_analyze:
    ...
    async def test_call_split_time_series_detail_with_minimal_args(
        ...
        with patch(
            "garmin_mcp.rag.queries.time_series_detail.TimeSeriesDetailExtractor.__init__",
            mock_init_detail,
        ):
            result = await call_tool(
    ...
    async def test_call_split_time_series_detail_with_metrics_filter(
        ...
        with patch(
            "garmin_mcp.rag.queries.time_series_detail.TimeSeriesDetailExtractor.get_split_time_series_detail"
        ) as mock_detail:
    ...
    async def test_call_get_form_anomaly_details_with_filters(
        ...
        with patch(
            "garmin_mcp.rag.queries.form_anomaly_detector.FormAnomalyDetector.get_form_anomaly_details"
        ) as mock_details:
    ...
    async def test_call_analyze_performance_trends_with_minimal_args(
        ...
        with patch(
            "garmin_mcp.rag.queries.trends.PerformanceTrendAnalyzer.analyze_metric_trend"
        ) as mock_analyze:
            mock_analyze.return_value = {
                "metric": "pace",
                "trend": "improving",
    ...
    async def test_call_extract_insights_general_search(self):
        ...
        with patch(
            "garmin_mcp.rag.queries.insights.InsightExtractor.search_by_keywords"
        ) as mock_search:
    ...
    async def test_call_extract_insights_single_activity(
        ...
        with patch(
            "garmin_mcp.rag.queries.insights.InsightExtractor.extract_insights"
        ) as mock_extract:
    ...
    async def test_call_compare_similar_workouts_with_minimal_args(
        ...
        with patch(
            "garmin_mcp.rag.queries.comparisons.WorkoutComparator.find_similar_workouts"
        ) as mock_find:
