...
**Units & notes**: `pace_consistency` is the coefficient of variation (stdev/mean) of pace over **representative** run laps only — laps whose distance is below 0.5x the median run-lap distance (GPS fragment laps) are excluded so a tiny trailing lap cannot inflate it (#852). The fragment-inclusive raw CV (`pace_consistency_full`) was removed in #972 — including fragment laps made the value meaningless and it was being graded as if it were the real consistency figure. `hr_drift_percentage` (%); `*_splits` columns are comma-separated split indices; `*_avg_pace_seconds_per_km` (sec/km); `*_avg_pace_str` (mm:ss); `*_avg_hr` / `*_avg_cadence` / `*_avg_power` are per-phase averages (power NULL when no power data). The four phase prefixes are `warmup` / `run` / `recovery` / `cooldown`, each with a `*_evaluation` quality string.
...
**Units & notes**: `user_id` defaults to `'default'`, `condition_group` to `'flat_road'`; `metric` is `gct` / `vo` / `vr`; `model_type` distinguishes power vs linear. GCT power-model coefficients `coef_alpha` (α, `log(v)` intercept) and `coef_d` (exponent, d < 0); VO/VR linear-model `coef_a` (intercept) and `coef_b` (slope). `period_start`/`period_end` bound the (inclusive) training window; `n_samples` is the sample count; `rmse` the error; `speed_range_min`/`speed_range_max` (m/s) — the band the model can be **trusted** over: the 5-95 percentile of the training speeds, not their raw min/max (#1096). Predictions outside it inflate the metric's sigma so the star rating stays conservative, and the evaluation carries `extrapolated: true` (#1088). Percentiles rather than extremes because a baseline covering an activity may be trained on that activity's own splits, so one fast workout would otherwise stretch the range past everything else and silence the guard. Below 20 samples, or when the percentile band collapses, the raw min/max is stored instead. `power_a`/`power_b` are the speed-from-power coefficients and `power_rmse` its error.
...
`review_data` keys (free-form JSON, no migration needed to extend): `this_week` (W-1 actuals summary), `prev_week_adherence` (W-1 prescription counts, copied from the prefetch bundle), `block_alignment` (how W sits in its training block), `periodization` (`expected_phase` / `block_phase` / `ladder_step_km` / `weeks_to_block_end` / `gap` — the gap is measured against the **stored training block**, not the Garmin adaptive plan), `recovery`, `goal_alignment`, `recommendations`, `overall`, `revision_note` (why a mid-week revision was issued), plus the Garmin-side `garmin_conflicts` (`{date, garmin_title, reason}` — only calendar items that contradict the block) and `garmin_next_week`, which is kept for the Web detail page but filled from the conflict list only. `save_weekly_review` returns the new `review_id`, which `/weekly-review` passes to `save_weekly_prescriptions` so the week's structured rows (`weekly_prescriptions.review_id`) point back at this exact review version.

`verdict` (one row per prescribed session) is **not stored**: saving a non-empty `review_data.verdict` is rejected, and the readers derive it from `weekly_prescriptions` — the batch linked to this `review_id`, else the week's canonical (MAX `batch_id`) batch, else the stored payload for reviews written before the split (issue #1021). The derived rows carry `{date, session, rating, comment, session_type, target_km, target_minutes, hr_low, hr_high, status, prescription_id}` alongside `verdict_source` (`prescriptions` / `stored`) and `prescription_batch_id`.
