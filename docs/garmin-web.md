...
|-------|-----------|----------|--------------|
| `/` | ホーム | 今日どう動く? | Verdict line (判定 + 今日の処方) → vitals row (HRV / RHR / 睡眠・準備度 / ACWR, each deep-linking into `/condition`) → this week's 7-day strip + coach note → progress (featured race countdown, last run — 前回ランの処方判定・注意フラグとコーチ所見 1 文) |
...
|-------|--------------|---------|
| `/activities/:id` | activity list, recent runs | One run, read as the reader's questions (no stars, no points): header (meta line, run name, 処方の判定と注意フラグ, KPI row, 前回比) → コーチの総評 (`run_note`: story / 良かった点 / 伸ばせる点 / 持ち越す 1 点 / 次のセッション (`report.next_session`, only when the plan names it) / 確認したいこと) → ランの流れ (stacked pace + HR panels drawn from `report.flow`: a real distance or elapsed-time axis, one step per drawn split, numbered scene bands from `report.moments` captioned in a row above the plot) → 計画との照合 (`report.plan.checks` + 心拍上限の超過) → いつもと比べて (`report.signals` against the athlete's own normal range + HR zones) → 記録 (conditions, time series, GPS track, the splits the report drew with a 場面 column — a 区間 table first on a multi-step session, its raw laps folded behind a disclosure — phases, `以前の分析（旧形式）` disclosure)。フォーム異常のあるスプリット行を強調（`/split-anomalies`）、過去分析の版切り替えあり |
...
- **No duplicated answers.** A number is owned by exactly one page: the home
  ...
  prediction sits in the `/goal` A / B race column rather than in a second card
  below it (#894, #895).
