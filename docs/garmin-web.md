...
|-------|--------------|---------|
| `/activities/:id` | activity list, recent runs | One run, read as the reader's questions (no stars, no points): header (meta line, run name, 処方の判定と注意フラグ, KPI row, 前回比) → コーチの総評 (`run_note`: story / 良かった点 / 伸ばせる点 / 次回のチャレンジ + `next_run_target` / 確認したいこと) → ランの流れ (stacked pace + HR panels drawn from `report.flow`: a real distance or elapsed-time axis, one step per drawn split, numbered scene bands from `report.moments` captioned in a row above the plot) → 計画との照合 (`report.plan.checks` + 心拍上限の超過) → いつもと比べて (`report.signals` against the athlete's own normal range + HR zones) → 記録 (conditions, time series, GPS track, the splits the report drew with a 場面 column — a 区間 table first on a multi-step session, its raw laps folded behind a disclosure — phases, `以前の分析（旧形式）` disclosure)。フォーム異常のあるスプリット行を強調（`/split-anomalies`）、過去分析の版切り替えあり |
