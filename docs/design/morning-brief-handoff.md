...
3. **BlockBands** → 1 本の帯: `grid grid-cols-[96px_repeat(7,1fr)]` の 2 列目以降に `grid-cols-{daysInView}` で日単位のスパン。ビルド系 = ~~`bg-ink text-paper`~~（**#1172 で改訂**: ビルド系の黒ベタは通常状態がページ最強の要素になり「色は例外だけ」の原則と逆転するため、`border border-hairline bg-well text-ink` の淡色地・罫線帯に変更。未知フェーズの fallback も `border border-hairline text-ink-muted` へ、ビルド系と区別できるよう塗りなしにした）、カットバック/テーパー = `bg-warn-tint text-status-warn border border-warn-line`、レース = `bg-bad-tint text-status-bad`（この2つは変更なし）。高さ 22、mono 11 tracking、内容 `{phase} · {title} · MM/DD – MM/DD · ポイント練 週{n} · 体重 {mode}`。
