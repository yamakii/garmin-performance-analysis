...
| `--tmpfs /tmp:size=8g` (#1078) | per-test DuckDB copies stalling in checkpoint + fsync on the overlay `/tmp` — the suite ran 44 s or 150 s at random (the high I/O-pressure reading at the time was ghostty's io_uring, not the disk) | anything: it is a performance setting. 8 GiB is a cap charged to `--memory`, not a reservation; pytest keeps only failed tests' temp dirs |
